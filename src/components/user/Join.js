...
const Join = () => {
  ...
  // 상태값 변경에 의해 화면의 재 렌더링이 발생할때, 컴포넌트의 함수들도 재 선언이 됩니다.
  ...
  const debouncedUpdateState = useCallback(
    ...
    [],
  ); // 의존성 배열을 비워놓으면 처음 랜더링될때만 함수가 선언되고 이후에 바뀌지않음.(재선언되지않음)
  // 만약 함수의 선언이 특정 상태가 변할 때 재선언 되어야 한다면, 의존성 배열에 상태 변수를 선언 하시면 됩니다.
  ...
  const fetchSignUpPost = async () => {
    ...
    */
    ...
    if (res.status === 200) {
      ...
      alert(
        `${data.userName}(${data.email})님 회원가입 성공!`,
      );
      // 로그인 페이지로 리다이렉트
      navigate('/login');
    } else {
      alert(
        '서버와의 통신이 원활하지 않습니다. 관리자 문의 gogo',
      );
    }
  ...
  const showThumbnailHandler = (e) => {
    ...
    if (
      ...
      alert(
        '이미지 파일(jpg, png, jpeg, gif)만 등록 가능합니다!',
      );
      // 형식에 맞지않는 파일을 첨부한 것이 파악되었다면 input 상태도 원래대로 돌려놓아야한다.
